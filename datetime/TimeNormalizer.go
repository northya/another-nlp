...
func NewTimeNormalizer() (timeNor *TimeNormalizer){
	fmt.Println(timeExp)
	timeNor = &TimeNormalizer{
		re: regexp.MustCompile(timeExp),
	}
	return
}

func (tn *TimeNormalizer) getTimeBase() string {
	return tn.timeBase
}

func (tn *TimeNormalizer) Parse(target string ) ([]byte, error){
	tn.target = target
	tn.isTimeSpan = false
	tn.invalidSpan = false
	tn.timeSpan = ""
	tn.timeBase = time.Now().Format("2006-01-02-15-04-05")
	tn.timeToken = tn.timeEx()
	res := tn.timeToken
	fmt.Println(res)
	...
	if tn.isTimeSpan {
		if tn.invalidSpan {
			...
			index := strings.Index(dic["timedelta"].(string), "days")

			days, err :=  strconv.Atoi(dic["timedelta"].(string)[:index - 1])
			if err != nil {
				panic(err)
			}

			//days := gjson.Get(dic["timedelta"].(string), "days").Int()
			result["year"] = days / 365
			result["month"] = days / 30 - int(result["year"].(int)) * 12
			result["days"] = days - result["year"].(int) * 365 - result["month"].(int) * 30
			index = strings.Index(dic["timedelta"].(string), ",")
			timeSpan := dic["timedelta"].(string)[index + 1:]
			timeSpans := strings.Split(timeSpan, ":")
			result["hour"] = timeSpans[0]
			result["minute"] = timeSpans[1]
			result["second"] = timeSpans[2]
			dic["timedelta"] = result

		}
	} else {
		if len(res) == 0 {
			dic["error"] = "no time pattren could be extracted"
		} else if len(res) == 1 {
			dic["type"] = "timestamp"
			dic["timestamp"] = res[0].time.Format("2006-01-02 15:04:05")
		} else {
			dic["type"] = "timestamp"
			dic["timespan"] = res[0].time.Format("2006-01-02 15:04:05") + "," +
				"" + res[1].time.Format("2006-01-02 15:04:05")
		}
	}
	return json.Marshal(dic)
}

func (tn *TimeNormalizer) timeEx() (res []*TimeUnit) {
	startLine := -1
	endLine := -1
	rpointer := 0

	match := tn.re.FindAllString(tn.target, -1)
	matchIndex := tn.re.FindAllStringIndex(tn.target, -1)
	temp := make([]string, len(match))

	//startMark := true
	for i := 0; i < len(match); i++ {

		startLine = matchIndex[i][0]
		if endLine == startLine {
			rpointer--
			temp[rpointer] = temp[rpointer] + match[i]
		} else {
			temp[rpointer] = match[i]
			//temp = append(temp, match[i])
		}
		endLine = matchIndex[i][1]
		rpointer++
	}
	contextTp := NewTimePoint()
	fmt.Println("timebase:", tn.timeBase)
	fmt.Println("temp:", temp)
	for i := 0; i < rpointer; i++ {
		fmt.Printf("temp %d : %s, %d\n", i, temp[i], len(temp))
		if temp[i] == "" {
			continue
		}
		res = append(res, NewTimeUnit(temp[i], tn, contextTp))
		contextTp = res[i]._tp
	}

	res = tn.fileterTimeUnit(res)

	return
}

func (tn *TimeNormalizer) fileterTimeUnit(tus []*TimeUnit) []*TimeUnit {
	/*
	过滤timeUnit中无用的识别词，无用识别词识别出的时间是1970.01.01 00:00:00
	*/
	if len(tus) < 1 {
		return tus
	}
